...
/proc/CommonDoorAnimator(dmifile as icon)
	if(closing)
		...
		for(FRAME_N2 = 13, FRAME_N2 > 0, FRAME_N2--)
			...
			lframe.DrawBox(null, 17, 32, 32, 27)
			lframe.DrawBox(null, 18, 26, 32, 21)
			lframe.DrawBox(null, 17, 20, 32, 13)
			lframe.DrawBox(null, 18, 12, 32, 7)
			lframe.DrawBox(null, 17, 6, 32, 1)
			lframe.DrawBox(null, 1, 32, FRAME_N1, 1)
			lframe.Shift(WEST, FRAME_N1)
			var/icon/rframe = icon(base_door)
			rframe.DrawBox(null, 1, 32, 15, 27)
			rframe.DrawBox(null, 1, 26, 16, 21)
			rframe.DrawBox(null, 1, 20, 15, 13)
			rframe.DrawBox(null, 1, 12, 16, 7)
			rframe.DrawBox(null, 1, 6, 15, 1)
...
/proc/CommonDoorOpenState(dmifile as icon)
...
/proc/CommonDoorLockedState(dmifile as icon)
	base_door = icon(door_state)
	base_door.Blend(CDred,ICON_OVERLAY,1,1)
	outputIcon.Insert(base_door, statename)
	fcopy(outputIcon, "Output/[filename]")

//secondary proc for generating deny door animation
/proc/CommonDoorDenyState(dmifile as icon)
	base_door = icon(door_state)
	base_door.Blend(CDred,ICON_OVERLAY,1,1)
	base_door.Blend(CDpanelred,ICON_OVERLAY,1,1)
...
/proc/CommonDoorSparksState(dmifile as icon)
	base_door = icon(door_state)
	base_door.Blend(CDsparks1,ICON_OVERLAY,1,1)
	outputIcon.Insert(base_door, statename, frame = 1)
	base_door = icon(door_state)
	base_door.Blend(CDpanelgreen,ICON_OVERLAY,1,1)
	base_door.Blend(CDgreen,ICON_OVERLAY,1,1)
	base_door.Blend(CDsparks2,ICON_OVERLAY,1,1)
	outputIcon.Insert(base_door, statename, frame = 2)
	base_door = icon(door_state)
	base_door.Blend(CDsparks3,ICON_OVERLAY,1,1)
	outputIcon.Insert(base_door, statename, frame = 3)
	base_door = icon(door_state)
	base_door.Blend(CDsparks4,ICON_OVERLAY,1,1)
	outputIcon.Insert(base_door, statename, frame = 4)
	base_door = icon(door_state)
	base_door.Blend(CDpanelred,ICON_OVERLAY,1,1)
	base_door.Blend(CDred,ICON_OVERLAY,1,1)
	base_door.Blend(CDsparks5,ICON_OVERLAY,1,1)
